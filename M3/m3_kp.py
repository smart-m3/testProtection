...
AR_NameSpace = "http://ProtectionOntology.org"
AR_Property  = AR_NameSpace+"#Has_Access_Restriction"
AR_Owner     = AR_NameSpace+"#Has_Owner"            
AR_Target    = AR_NameSpace+"#Has_Target" 
...
class KP:
	...
	print "Connections open at leave: ",self.connections
	if self.connections != []:
		for sub in self.connections:
			print "close connection: ",sub[1]
			sub[1].close()
    ...
    def InsertProtection(self, Entity, Properties,SmartSpace):
	Protection_Entity = "http://"+str(uuid.uuid4())

	triple = [(URI(Entity), URI(AR_Property), URI(Protection_Entity)),
			(URI(Protection_Entity), URI(AR_Owner),URI(self.node_id))]

	for prop in Properties:
		triple.append((URI(Protection_Entity), URI(AR_Target), Literal(prop)))

	q = self.CreateInsertTransaction(SmartSpace)
	status = q.send(triple)
	self.CloseInsertTransaction(q)
	return status[0]

    def RemoveProtection(self, Entity, Properties,SmartSpace):
	Protection_Entity = "http://"+str(uuid.uuid4())
	...
	q = self.CreateRemoveTransaction(SmartSpace)
	status = q.remove(triple)
	self.CloseRemoveTransaction(q)
	return status
